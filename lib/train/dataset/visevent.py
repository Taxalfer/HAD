...
class VisEvent(BaseVideoDataset):
    ...
    def get_start_frame_id(self, seq_id):
        seq_path = self._get_sequence_path(seq_id)
        frame_list = os.listdir(seq_path)
        sorted_frame_list = sorted(frame_list, key=lambda x: int(x[5:9]))
        start_frame_id = int(sorted_frame_list[0][5:9])
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        start_frame_id = self.get_start_frame_id(seq_id)
        frame_ids = [f_id + start_frame_id for f_id in frame_ids]
